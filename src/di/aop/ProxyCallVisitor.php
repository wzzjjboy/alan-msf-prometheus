...
class ProxyCallVisitor extends NodeVisitorAbstract
{
    ...
    private function rewriteMethod(ClassMethod $node): ClassMethod
    {
        ...
        $staticCall = new MethodCall(new Variable('this'), '__proxyCall', [
            // __CLASS__
        ...
        if ($shouldReturn) {
            $stmts[] = new Return_(new Node\Expr\Yield_($staticCall));
        } else {
            $stmts[] = new Expression(new Node\Expr\Yield_($staticCall));
        }
        ;
        $node->stmts = $stmts;
        return $node;
    ...
    private function shouldRewrite(ClassMethod $node)
    {
        ...
        if (strpos($node->name->name, 'action') !== 0) {
            return false;
        }
        return true;
    }
